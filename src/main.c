...
int main( int argc, char *argv[] ) {
    ...
    if ( JSON_ARRAY != json_typeof( json ) ) {
        ...
        for ( int x = 0; x < size; x++ ) {
            psvDebugScreenPrintf( "%s\n",                 homebrews[ x ].name );
            psvDebugScreenPrintf( "TITLE_ID:       %s\n", homebrews[ x ].title_id );
            psvDebugScreenPrintf( "Description:    %s\n", homebrews[ x ].description );
            psvDebugScreenPrintf( "Icon:           %s\n", homebrews[ x ].icon );
            psvDebugScreenPrintf( "Latest version: %s\n", homebrews[ x ].latest );
            psvDebugScreenPrintf( "Latest URL:     %s\n", get_latest_url( homebrews[ x ] ) );
            psvDebugScreenPrintf( "\n" );

            for ( int y = 0; y < homebrews[ x ].numScreenshots; y++ ) {
            psvDebugScreenPrintf( "Screenshot:     %s\n", homebrews[ x ].screenshots[ y ] );
            }

            psvDebugScreenPrintf( "\n" );

            for ( int y = 0; y < homebrews[ x ].numVersions; y++ ) {
            psvDebugScreenPrintf( "Version:        %s\n", homebrews[ x ].versions[ y ]->version );
            psvDebugScreenPrintf( "URL:            %s\n", homebrews[ x ].versions[ y ]->url );
            }
            psvDebugScreenPrintf("-----------------------------------------\n");
        }
    }
